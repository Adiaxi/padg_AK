...
# BANKS
    ...
    def build_bank_form_edit_window(self):
        ...
        self.entry_bank_id = ctk.CTkEntry(self.window_form_frame)
        self.entry_bank_id.insert(0, self.selected_bank.id)
        ...
        self.button_bank_save.grid(row=7, column=0, columnspan=2,padx=30,pady=25, sticky="nsew")
...
# WORKERS
    ...
    def build_worker_form_edit_window(self):
        ...
        self.entry_worker_bank.insert(0, self.selected_worker.bank)
        ...
        for i, entry in enumerate(entries, start=1):
            entry.grid(row=i, column=1, padx=15, pady=5, sticky='nsew')
        self.button_worker_save = ctk.CTkButton(self.window_form_frame, text="Save", font=('Montserrat', 20, "bold"),width=120, height=35, command=lambda: (add_worker(self.entry_worker_name.get(), self.entry_worker_surname.get(), self.entry_worker_bank.get(),self.entry_worker_role.get(), self.entry_worker_town.get(), self.entry_worker_street.get(),self.entry_worker_number.get(), self.entry_worker_photo.get(),self.entry_worker_password.get()), self.worker_entry_clear(), self.worker_info()))
    ...
    def worker_info(self):
        ...
        for worker in workers:
            ...
            btn = ctk.CTkButton(self.row, text="X", font=("Montserrat", 11, "bold"), width=80, command=lambda temp=worker: (delete_worker(temp),self.worker_info()), fg_color="#B91C1C", hover_color="#DC2626")
            btn.pack(side='right', padx=10)
